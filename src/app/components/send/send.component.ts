...
export class SendComponent implements OnInit {
  ...
  syncNanoPrice() {
    ...
    const rawAmount = this.util.nano.mnanoToRaw(
      new BigNumber(fiatAmount).div(this.price.price.lastPrice)
    );
  ...
  async validateDestination() {
    ...
    this.addressBookMatch = this.addressBookService.getAccountName(
      this.toAccountID
    );
  ...
  async sendTransaction() {
    ...
    if (nanoAmount.lessThan(1))
      return this.notificationService.sendWarning(
        `Transactions for less than 1 banano will be ignored by the node.  Send raw amounts with at least 1 nano.`
      );
    if (from.balanceBN.minus(rawAmount).lessThan(0))
      return this.notificationService.sendError(
        `From account does not have enough NANO`
      );
    ...
    this.fromAddressBook = this.addressBookService.getAccountName(
      this.fromAccountID
    );
    this.toAddressBook = this.addressBookService.getAccountName(
      this.toAccountID
    );
